...
def judge_rule(rule: dict, extraction: dict) -> dict:
    ...
    for pass_cond in pass_conds:
        pass_signals = [w for w in pass_cond.lower().split() if len(w) > 4]
    ...
    # Check 5: Content found but neither clearly passes nor fails → manual review
    return {
        "rule_id":  rule_id,
        "severity": severity,
        "status":   "MANUAL_REVIEW",
...
def build_report(findings: list) -> str:
    ...
    if manuals:
        ...
        for f in manuals:
            lines += [
                f"Rule: {f.get('rule_id','')} [{f.get('severity','')}]",
